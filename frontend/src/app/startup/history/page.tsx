...
import { Search, Filter, Play, Brain, Download, FileText, Sparkles, Users } from "lucide-react";
...
export default function StartupHistoryPage() {
  ...
  const mentors = ["All", "Dr. Sarah Kim", "James Wilson"];

  const displayData = meetingHistory.filter(m => filterMentor === "All" || m.mentor === filterMentor);
      ...
      <div className="sticky top-0 z-40 bg-background/90 backdrop-blur-md -mx-6 px-6 pt-6 pb-4 mb-6 -mt-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 pr-[300px]">
            ...
            >
              {mentors.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            ...
            <div className="p-5 flex-1">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <Avatar name={m.mentor} size="sm" />
                  <div>
                    <h3 className="text-base font-bold text-foreground group-hover:text-primary transition-colors">{m.mentor}</h3>
          ...
          <div className="relative w-full max-w-4xl bg-surface border border-border shadow-2xl rounded-xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-border bg-surface-container/30 shrink-0">
              <div className="flex items-center gap-4">
                <Avatar name={selectedMeeting.mentor} size="lg" />
                <div>
                  <h2 className="text-xl font-bold text-foreground">{selectedMeeting.type}</h2>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-sm font-medium text-foreground">{selectedMeeting.mentor}</span>
